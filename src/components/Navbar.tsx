...
const Navbar: React.FC = () => {
  ...
  return (
    <motion.header 
      ...
      className="sticky top-0 w-full z-50 bg-surface/80 dark:bg-dark-surface/80 backdrop-blur-md border-b border-outline-variant/10 dark:border-[#3a3a55]/30"
    >
      <div className="flex justify-between items-center px-margin-mobile py-4 max-w-[1440px] mx-auto md:px-margin-desktop">
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <a 
              ...
              className="text-sm font-bold font-plus-jakarta text-on-surface-variant dark:text-dark-on-surface-variant hover:text-primary transition-colors uppercase tracking-widest"
            >
              {link.name}
            </a>
          ))}
        ...
        <div className="flex items-center gap-2">
          <button 
            ...
            className="p-2 rounded-full hover:bg-primary/5 transition-all text-on-surface-variant dark:text-dark-on-surface-variant"
          >
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div
            ...
            className="md:hidden bg-surface dark:bg-dark-surface border-b border-outline-variant/10 dark:border-[#3a3a55]/30 overflow-hidden"
          >
            <div className="flex flex-col p-4 gap-4">
              {navLinks.map((link) => (
                <a 
                  ...
                  href={link.href}
                  onClick={(e) => handleNavClick(e, link.href)}
                  className="text-lg font-bold font-plus-jakarta text-on-surface-variant dark:text-dark-on-surface-variant hover:text-primary transition-colors py-2"
                >
                  {link.name}
                </a>
              ))}
